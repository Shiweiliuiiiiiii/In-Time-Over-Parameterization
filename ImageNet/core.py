                ...
                for name, weight in module.named_parameters():
                    ...
                    if self.redistribution_mode == 'none':
                        ...
                        if self.args.grow_switch:
                            reini_value = float(torch.mean(torch.abs(weight[(new_mask - self.masks[name].data.byte())])))
                            ...
                            with torch.no_grad():
                                ...
                                weight[(new_mask - self.masks[name].data.byte())] = torch.sign(
                                    grad[(new_mask - self.masks[name].data.byte())]) * torch.FloatTensor(int(num)).uniform_(0, reini_value).cuda()
